...
-- Вывести кол/во сотрудников из департаментов 10, 11, 9, 8 по отдельности.

select department_id, count(employee_id)
from employees
where department_id in (10, 11, 9, 8)
group by department_id;

-- Вывести кол/во сотрудников в каждом департаменте, если кол/во больше 3.
...
-- Посчитать кол/во сотурников в каждом департаменте. 
-- Вывести имя департамента и кол/во сотрудников.
select t1.department_name, count(t2.employee_id) as emp_count
from departments t1
join employees t2
on t1.department_id = t2.department_id
group by t1.department_name;

-- Вывести имена департаментов и минимальные зарплаты в этих департаментах, 
-- если они больше 5000.
...
-- Найти сотрудников с минимальной зарплатой каждой должности(job_id).
-- Вывести first_name, last_name, salary, job_id
...
-- Вывести city_count(as), country_id
...
select count(t1.city) as city_count, t2.country_name
from locations t1
join countries t2
...
-- Найти департаменты, в которых больше 6 сотрудников. Вывести имя департамента.
select t1.department_name
from departments t1
join ( select department_id, count(*) as emp_count
				from employees
                group by department_id
                having emp_count > 6) t2
on t1.department_id = t2.department_id;
